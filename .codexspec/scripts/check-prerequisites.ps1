...
# Consolidated prerequisite checking script (PowerShell) for CodexSpec
#
...
Consolidated prerequisite checking for CodexSpec workflow.
...
if (-not (Test-Path $paths.FEATURE_DIR -PathType Container)) {
    ...
    Write-Output "Run /codexspec:specify first to create the feature structure."
    exit 1
}
...
if ($RequireTasks -and -not (Test-Path $paths.TASKS -PathType Leaf)) {
    ...
    Write-Output "Run /codexspec:plan-to-tasks first to create the task list."
    exit 1
}
